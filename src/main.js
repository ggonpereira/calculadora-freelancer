...
let cardText = document.querySelector("#card-info");
...
function calculate() {
  ...
  let daysWeek = document.getElementById("days-week").value;
  let projectHoliday = document.getElementById("project-holiday").value;
  let projectCost = document.getElementById("project-cost").value;

  let costHour =
    projectCost / (daysWeek * 4 * hoursDay) +
    projectHoliday * daysWeek * hoursDay;

  let resultShow = `Seu valor/hora é: R$${costHour.toFixed(2)}`;

  let newContent = document.createTextNode(resultShow);
...
function editInfo(title, info) {
  ...
  let titleCard = title;
  let newTitle = document.createTextNode(titleCard);
  let infoCard = info;
  let newInfo = document.createTextNode(infoCard);
